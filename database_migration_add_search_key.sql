...
-- 2. Criar índice para busca rápida usando search_key
CREATE INDEX IF NOT EXISTS idx_subtopic_lessons_search_key
ON public.subtopic_lessons(search_key);

-- 3. Criar índice para busca por subject e education_level
CREATE INDEX IF NOT EXISTS idx_subtopic_lessons_subject_level
...
-- Nota: Como não temos subject e education_level nos registros antigos,
-- eles precisarão ser atualizados manualmente ou através da aplicação
UPDATE public.subtopic_lessons
SET search_key = COALESCE(subject, 'unknown') || '::' ||
                 COALESCE(education_level, 'unknown') || '::' ||
                 module_index || '::' ||
                 topic_index || '::' ||
                 subtopic_index
WHERE search_key IS NULL;

-- 8. Adicionar constraint única mais flexível
-- Remover a constraint antiga se existir
ALTER TABLE public.subtopic_lessons
DROP CONSTRAINT IF EXISTS subtopic_lessons_unique_path;

-- Adicionar nova constraint usando search_key
ALTER TABLE public.subtopic_lessons
ADD CONSTRAINT subtopic_lessons_unique_search_key
UNIQUE (search_key);

-- 9. Criar função para buscar aulas por padrão
...
-- 10. Criar view para facilitar visualização
...
ORDER BY subject, education_level, module_index, topic_index, subtopic_index;
